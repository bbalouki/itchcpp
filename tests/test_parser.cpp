...
std::vector<char> to_big_endian(T val) {
    ...
    const char* bytes = reinterpret_cast<const char*>(&value);  // NOLINT
...
std::string create_test_buffer(const T& msg) {
    ...
    std::vector<char> b_length = to_big_endian(length);
    std::vector<char> buffer;
    buffer.insert(buffer.end(), b_length.begin(), b_length.end());

    const char* msg_bytes = reinterpret_cast<const char*>(&msg);  // NOLINT
...
std::string create_test_buffer(const itch::SystemEventMessage& msg) {
    ...
    std::string buffer(b_length.begin(), b_length.end());
...
TEST_F(ParserTest, SingleValidSystemEventMessage) {
    ...
    auto messages = parser.parse(data_stream);
...
TEST_F(ParserTest, MultipleValidMessages) {
    ...
    std::stringstream data_stream(data1 + data2);
...
TEST_F(ParserTest, CallbackBasedParsing) {
    ...
    parser.parse(data_stream, callback);
...
TEST_F(ParserTest, IgnoresTrailingGarbageData) {
    ...
    EXPECT_THROW(messages = parser.parse(data_stream), std::runtime_error);
}
...
TEST_F(ParserTest, HandlesStreamEndingExactlyOnBoundary) {
    ...
    std::vector<itch::Message> messages;
    EXPECT_NO_THROW(messages = parser.parse(data_stream));
    ASSERT_EQ(messages.size(), 2);
}
