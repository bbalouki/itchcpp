...
struct Order {
    ...
    uint32_t shares;
    uint32_t price;
    PriceLevel* level; // Pointer to the price level this order belongs to

    Order(uint64_t ref_num, char side, uint32_t qty, uint32_t prc)
        : order_reference_number(ref_num), buy_sell_indicator(side), shares(qty), price(prc), level(nullptr) {}
};

// Represents a single price level in the order book.
// It contains a list of orders at that price, in time priority.
struct PriceLevel {
    uint32_t total_shares = 0;
    std::list<std::shared_ptr<Order>> orders;

    void add_order(std::shared_ptr<Order> order);
    void remove_order(std::list<std::shared_ptr<Order>>::iterator order_it);
};
...
private:
    ...
    // The core data structures for the order book
    BidMap m_bids;
    AskMap m_asks;
    std::map<uint64_t, std::list<std::shared_ptr<Order>>::iterator> m_orders;
};
