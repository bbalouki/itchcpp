...
auto BookManager::process(const Message& message) -> void {
    if (const auto* add = std::get_if<AddOrderMessage>(&message)) {
        ...
        if (target != nullptr) {
            target->book.add_order(
                ...
                to_side(add->buy_sell_indicator),
                add->shares,
                add->price
            );
    ...
    if (const auto* add = std::get_if<AddOrderMPIDAttributionMessage>(&message)) {
        BookEntry* target = ensure_entry(add->stock_locate, to_string(add->stock, STOCK_LEN));
        if (target != nullptr) {
            target->book.add_order(
                add->order_reference_number,
    ...
    if (const auto* exec = std::get_if<OrderExecutedMessage>(&message)) {
    ...
    if (const auto* exec = std::get_if<OrderExecutedWithPriceMessage>(&message)) {
        BookEntry* target = entry(exec->stock_locate);
        if (target == nullptr) {
            return;
        }
    ...
    if (const auto* cancel = std::get_if<OrderCancelMessage>(&message)) {
    ...
    if (const auto* del = std::get_if<OrderDeleteMessage>(&message)) {
    ...
    if (const auto* replace = std::get_if<OrderReplaceMessage>(&message)) {
    ...
    if (const auto* trade = std::get_if<NonCrossTradeMessage>(&message)) {
    ...
    if (const auto* directory = std::get_if<StockDirectoryMessage>(&message)) {
